...
def get_tenure_risk(tenure):
    """Classify tenure into risk tiers based on historical churn patterns."""
    if tenure <= 6:
...
def get_charges_risk(monthly_charges):
    """Classify monthly charges into risk tiers."""
...
if model_loaded:
    ...
    if tenure > 0 and total_charges < min_expected_total and total_charges > 0:
        st.warning(
            ...
            f"{tenure} months at ${monthly_charges:.0f}/month. "
            f"Expected at least ~${min_expected_total:.0f}. "
        ...
        try:
            ...
            if high_risk_factors:
                ...
                for factor in high_risk_factors:
                    st.markdown(
                        f"""
                        <div style="background-color: #fee2e2; color: black; padding: 10px;
                             border-radius: 5px; margin: 5px 0; border-left: 4px solid #ef4444;">
                            <b>⚠️ {factor['feature']}: {factor['value']}</b><br>
                            <small>Historical churn rate: {factor['churn_rate']}
                            | 💡 {factor['suggestion']}</small>
                        </div>
                        """,
                ...
                with st.expander(f"🟡 Medium Risk Factors ({len(medium_risk_factors)})"):
                    for factor in medium_risk_factors:
                        st.markdown(
                            ...
                            f"Churn rate: {factor['churn_rate']} | 💡 {factor['suggestion']}"
                        )
            ...
            if high_risk_factors:
                ...
                st.success(
                    ...
                    "Customer has a good retention profile."
                )
